...
def restore():
    ...
    # Find the da_mock3 key
    mock3_key = '"da_mock3":'
    json_mock3 = json.dumps(DA_MOCK3, indent=8)
    
    if mock3_key in content:
        # Find start of array [
        start_idx = content.find('[', content.find(mock3_key))
        # Find corresponding closing ]
        if start_idx != -1:
             # Just replace the empty array if found
             empty_pattern = r'"da_mock3":\s*\[\s*\]'
             if re.search(empty_pattern, content):
                  new_content = re.sub(empty_pattern, f'"da_mock3": {json.dumps(DA_MOCK3, indent=8)}'.replace('\\', '\\\\'), content)
             else:
                  # Use a simpler approach: find the mock3 line and replace the block
                  # We'll use the logic from apply_mocks_safe.py if needed, 
                  # but let's try a simpler string replace first.
                  print("da_mock3 exists and might not be empty. Rebuilding content...")
                  parts = re.split(r'"da_mock3":\s*\[.*?\]', content, flags=re.DOTALL)
                  if len(parts) == 2:
                       new_content = parts[0] + f'"da_mock3": {json_mock3}' + parts[1]
                  else:
                       print("Regex split failed. Trying simple replace.")
                       new_content = content.replace('"da_mock3": []', f'"da_mock3": {json_mock3}')
             
             with open(path, "w", encoding="utf-8") as f:
                f.write(new_content)
             print("Successfully restored da_mock3.")
        else:
            print("Could not find start of array for da_mock3.")
    else:
        print("da_mock3 key not found at all.")
