...
# Check if da_mock3 already exists
if 'da_mock3' in content:
    print("da_mock3 already exists. Restoration might have partially succeeded or content is different.")
    # I'll proceed only if it looks broken or if I want to overwrite.
    # Actually, if it's already there, I'll assume it's good or I'll just replace it.

with open(r'Quiz DD/extracted_da_mock3.json', 'r') as f:
    orig_mock3 = json.load(f)["da_mock3"]

q1_20 = orig_mock3[0:20]
q34_44 = orig_mock3[33:44]
...
full_mock3 = q1_20 + q21_33 + q34_44
da_mock3_str = '"da_mock3": ' + json.dumps(full_mock3, indent=8)

# Robust end pattern handling trailing whitespace
end_pattern = re.compile(r'\]\s*\}\s*;\s*$', re.DOTALL)
if end_pattern.search(content):
    new_content = end_pattern.sub('],\n    ' + da_mock3_str + '\n};', content)
    with open(path, 'wb') as f:
        f.write(new_content.encode('utf-16'))
    print("Restoration of 1-44 completed successfully.")
else:
    print("Could not find the expected end of dataQuizData object.")
